        ...
        Alamofire.request(URL_GET_DATA).responseJSON { response in
            ...
            if let json = response.result.value {
                ...
                for i in 0..<locationsArray.count{
                    ...
                    self.locations.append(Hero(
                        name: (locationsArray[i] as AnyObject).value(forKey: "name") as? String,
                        team: (locationsArray[i] as AnyObject).value(forKey: "vicinity") as? String,
                        imageUrl: (locationsArray[i] as AnyObject).value(forKey: "imageurl") as? String
                    ))
